...
class Runner(object):
    ...
    def _forward(model, batch):
        inputs, frame_level_targets, time, embedding, filenames, events, is_source = batch
        # print(events)
        ...
        is_source = convert_tensor(is_source, device=DEVICE, non_blocking=True)
        decision, decision_up, E = model(inputs,embedding)
        return decision, decision_up, frame_level_targets, time, E, is_source
    
    @staticmethod
    def _backward_D(model, E, is_source):
        return model.discriminator(E,is_source)
    ...
    def train(self, config, **kwargs):
        ...
        trainloader = dataset.getdataloader_s_urban_ad(
            config_parameters['train_data'], # feature path
            config_parameters['spk_emb_file_urban_path'],
            config_parameters['spk_emb_file_audioset_path'],
            transform=transform,
            batch_size=config_parameters['batch_size'],
            num_workers=config_parameters['num_workers'],**sampling_kwargs)

        cvdataloader = dataset.getdataloader_s_test_urban_ad(
        ...
        if config_parameters['optimizer'] == 'AdaBound':
            ...
            optimizer_G = getattr(torch.optim, config_parameters['optimizer'],)(model.s_student.parameters(), **config_parameters['optimizer_args']) # 加载 optimizer
            optimizer_D = getattr(torch.optim, config_parameters['optimizer'],)(model.disconv.parameters(), **config_parameters['optimizer_args']) # 加载 optimizer

        utils.pprint_dict(optimizer_G, logger.info, formatter='pretty')
        utils.pprint_dict(optimizer_D, logger.info, formatter='pretty')
        ...
        lambda_gan = config_parameters['lambda_gan']
        def _train_batch(_, batch):
            model.train()
            with torch.enable_grad():
                # optimizer.zero_grad()
                decision, decision_up, frame_level_target, time, E, is_source = self._forward(model, batch)  # output is tuple (clip, frame, target)
                # print('decision ',decision.shape)
                # print('frame_level_target ',frame_level_target.shape)
                # assert 1==2
                # first step updata discriminator
                is_source = is_source.squeeze()
                # print('is_source ',is_source.shape)
                # assert 1==2
                # print(is_source==1.0)
                # assert 1==2
                self._set_requires_grad(model.disconv,True)
                optimizer_D.zero_grad()
                loss_D = self._backward_D(model, E.detach(), is_source)
                loss_D.backward()
                optimizer_D.step()
                # second step update feature extrator
                self._set_requires_grad(model.disconv,False) # fix the discriminator

                optimizer_G.zero_grad()
                loss_gan = self._backward_D(model, E, is_source) # calculate loss
                decision = decision[is_source==1.0]
                frame_level_target = frame_level_target[is_source==1.0]
                loss_sed = criterion(decision,frame_level_target)
                loss = loss_gan*lambda_gan + loss_sed
                loss.backward()
                # Single loss
                optimizer_G.step()
                # print('loss_D ',loss_D)
        ...
        def thresholded_output_transform(output):
            ...
            decision = decision[is_source==1.0]
            frame_level_target = frame_level_target[is_source==1.0]
            frame_level_target = frame_level_target > 0.5
            frame_level_target = frame_level_target.long()
        ...
        metrics = {
            'Loss': losses.Loss_s_ad(criterion),  #reimplementation of Loss, supports 3 way loss 
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_G, **config_parameters['scheduler_args']) # using scheduler with learning rate
        #scheduler_D = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_D, **config_parameters['scheduler_args']) # using scheduler with learning rate
        #scheduler_s = [scheduler_G, scheduler_D]
    ...
    def evaluate(
        ...
        dataloader = dataset.getdataloader_s_test_urban_ad(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                inputs, frame_level_targets,time, embedding, filenames, events, _ = batch
                decision, decision_up, frame_level_target, time,  E, is_source = self._forward(model, batch) # 
                # mAP_tar.append(frame_level_target.detach().cpu().numpy().squeeze(0))
                # mAP_pred.append(decision.detach().cpu().numpy().squeeze(0))
                if is_source[0,0] == 0.0:
                    continue
