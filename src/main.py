...
class Runner(object):
    ...
    def _forward(model, batch):
        ...
        return decision,decision_up, clip_level_targets
    ...
    def train(self, config, **kwargs):
        ...
        def _train_batch(_, batch):
            ...
            with torch.enable_grad():
                optimizer.zero_grad()
                decision,decision_up, clip_level_target = self._forward(model, batch)
                # print('decision ',decision.shape)
                # print('frame_level_target ',frame_level_target.shape)
                # assert 1==2
                loss = criterion(decision[:,0],clip_level_target[:,0])
                loss.backward()
                # Single loss
                optimizer.step()
                return loss.item()
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                inputs, clip_level_targets, embedding, filenames,events = batch
                decision,decision_up, clip_level_targets = self._forward(model, batch) # 
