...
class Runner(object):
    ...
    def _forward(model, batch):
        ...
        decision,decision_up,logit,contrast_pairs = model(inputs,embedding,frame_level_targets)
        return decision,decision_up, frame_level_targets, time,embed_label,logit,contrast_pairs
    ...
    def train(self, config, **kwargs):
        ...
        criterion_clr = getattr(losses,config_parameters['loss_clr'])().to(DEVICE)
        def _train_batch(_, batch):
            model.train()
            with torch.enable_grad():
                optimizer.zero_grad()
                decision,decision_up, frame_level_target, time,embed_label,logit,contrast_pairs = self._forward(model, batch)  # output is tuple (clip, frame, target)
                ...
                loss_clr = criterion_clr(contrast_pairs)
                # loss_ce = criterion_ce(logit,embed_label)
                # loss = loss_sed + loss_ce
                loss = loss_sed + 0.1*loss_clr
                loss.backward()
                # Single loss
                optimizer.step()
                writter.add_scalar('loss_sed', loss_sed.cpu().item(), train_engine.state.epoch)
                writter.add_scalar('loss_clr', loss_clr.cpu().item(), train_engine.state.epoch)
        ...
        metrics = {
            'Loss': losses.Loss_join_clr(criterion_sed),  #reimplementation of Loss, supports 3 way loss 
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                ...
                decision,decision_up, frame_level_target, time,embed_label,logit,contrast_pairs = self._forward(model, batch) # 
