...
class Runner(object):
    ...
    def _forward(model, batch):
        ...
        decision_time, decision_up, pseudo_label = model(inputs,embedding)
        return decision_time, decision_up, pseudo_label, clip_level_targets
    ...
    def train(self, config, **kwargs):
        ...
        def _train_batch(_, batch):
            ...
            with torch.enable_grad():
                optimizer.zero_grad()
                decision_time, decision_up, pseudo_label, clip_level_target = self._forward(model, batch)
                ...
                # assert 1==2
                loss = criterion(decision_time, pseudo_label)
                loss.backward()
                # Single loss
                optimizer.step()
                return loss.item()
        ...
        metrics = {
            'Loss': losses.Loss_stage3(criterion),  #reimplementation of Loss, supports 3 way loss 
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                ...
                print('the_num_true ',the_num_true.shape)
