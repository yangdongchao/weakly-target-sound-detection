...
class Runner(object):
    ...
    def _forward(model, batch):
        ...
        w_decision_time,w_clip_decision,w_decision_up,w_out,s_out,s_decision_time,mask_loss = model(inputs,embedding,clip_level_targets)
        return w_decision_time,w_clip_decision,w_decision_up,w_out,s_out,s_decision_time, clip_level_targets, mask_loss
    ...
    def train(self, config, **kwargs):
        ...
        def _train_batch(_, batch):
            ...
            with torch.enable_grad():
                optimizer.zero_grad()
                w_decision_time,w_clip_decision,w_decision_up,w_out,s_out,s_decision_time, clip_level_target, mask_loss = self._forward(model, batch)
                ...
                # assert 1==2
                loss = loss_sed + 5*loss_dis + 0.1*loss_mask
                loss.backward()
                # Single loss
                optimizer.step()
                ...
                writter.add_scalar('loss_mask', loss_mask.cpu().item(), train_engine.state.epoch)
                return loss.item()
        ...
        metrics = {
            'Loss': losses.Loss_stage1_mask(criterion),  #reimplementation of Loss, supports 3 way loss 
