...
class Runner(object):
    ...
    def _forward(model, batch):
        inputs, frame_level_targets, time, embedding, filenames, events, clip_level_target = batch
        # print(events)
        ...
        clip_level_target = convert_tensor(clip_level_target,device=DEVICE, non_blocking=True)
        decision, decision_up, pseudo_label = model(inputs,embedding)
        return decision, decision_up, frame_level_targets, time, pseudo_label, clip_level_target
    ...
    def train(self, config, **kwargs):
        ...
        if config_parameters['label_reverse']:
        ...
        def _train_batch(_, batch):
            ...
            with torch.enable_grad():
                optimizer.zero_grad()
                decision_time, decision_up, frame_level_target, time, pseudo_label, clip_level_target = self._forward(model, batch)  # output is tuple (clip, frame, target)
        ...
        metrics = {
            'Loss': losses.Loss_s(criterion),  #reimplementation of Loss, supports 3 way loss 
    ...
    def evaluate(
        ...
        config_parameters['test_data'] = '/apdcephfs/share_1316500/donchaoyang/code2/data/feature_tsd/train_add_neg_final.h5'
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                inputs, frame_level_targets,time, embedding, filenames,events,clip_level_target = batch
                decision_time, decision_up, frame_level_target, time, pseudo_label, clip_level_target = self._forward(model, batch) # 
                ...
                frame_level_target = frame_level_target.detach().cpu().numpy()
                # print('frame_level_target ', frame_level_target)
                the_num_true = (pseudo_label == frame_level_target).sum()
                # print('the_num_true ',the_num_true)
                # assert 1==2
                tmp_rate = the_num_true/125.0
        ...
        print('avg_rate ', avg_rate/num_train)
        assert 1==2
