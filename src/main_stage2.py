...
class Runner(object):
    ...
    def _forward(model, batch):
        ...
        w_decision_time,w_clip_decision,w_decision_up = model(inputs,embedding)
        return w_decision_time, w_clip_decision, w_decision_up, clip_level_targets
    ...
    def train(self, config, **kwargs):
        ...
        def _train_batch(_, batch):
            ...
            with torch.enable_grad():
                optimizer.zero_grad()
                w_decision_time, w_clip_decision, w_decision_up, clip_level_target = self._forward(model, batch)
                # print('decision ',decision.shape)
                # print('frame_level_target ',frame_level_target.shape)
                # assert 1==2
                loss = criterion(w_clip_decision[:,0],clip_level_target[:,0])
                loss.backward()
                # Single loss
                optimizer.step()
                return loss.item()
        ...
        metrics = {
            'Loss': losses.Loss_stage2(criterion),  #reimplementation of Loss, supports 3 way loss 
