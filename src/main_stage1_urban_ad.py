...
class Runner(object):
    ...
    def _forward(model, batch):
        ...
        inputs, clip_level_targets, embedding, filenames, events, is_source = batch
        # print(events)
        ...
        w_decision_time,w_clip_decision,w_decision_up,w_out,s_out,s_decision_time,E_ad = model(inputs,embedding)
        return w_decision_time,w_clip_decision,w_decision_up,w_out,s_out,s_decision_time, clip_level_targets, E_ad, is_source 
    ...
    def train(self, config, **kwargs):
        ...
        trainloader = dataset.getdataloader_urban_ad(
        ...
        cvdataloader = dataset.getdataloader_urban_ad(
        ...
        if config_parameters['optimizer'] == 'AdaBound':
            ...
            optimizer_G = getattr(torch.optim, config_parameters['optimizer'],)(model.w_student.parameters(), **config_parameters['optimizer_args']) # 加载 optimizer
        ...
        def _train_batch(_, batch):
            ...
            with torch.enable_grad():
                w_decision_time,w_clip_decision,w_decision_up,w_out,s_out,s_decision_time, clip_level_target,E_ad,is_source = self._forward(model, batch)
                # print('decision ',decision.shape)
                # print('frame_level_target ',frame_level_target.shape)
                # assert 1==2
                is_source = is_source.squeeze()
                # the first step, update discriminator
                self._set_requires_grad(model.disconv,True)
                optimizer_D.zero_grad()
                loss_D = self._backward_D(model, E_ad.detach(), is_source)
                loss_D.backward()
                optimizer_D.step()

                # the second step, update w_student
                self._set_requires_grad(model.disconv,False) # fix the discriminator
                optimizer_G.zero_grad()
                loss_gan = self._backward_D(model, E_ad, is_source) # calculate loss
                ...
                # assert 1==2
                loss = loss_gan*lambda_gan + loss_sed + 5*loss_dis
        ...
        def thresholded_output_transform(output):
            ...
            is_source = is_source.squeeze()
            w_clip_decision = w_clip_decision[is_source==1.0]
            clip_level_target = clip_level_target[is_source==1.0]
        ...
        metrics = {
            'Loss': losses.Loss_stage1_urban_ad(criterion),  #reimplementation of Loss, supports 3 way loss 
    ...
    def evaluate(
        ...
        dataloader = dataset.getdataloader_urban_ad(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                inputs, clip_level_target, embedding, filenames,events,_ = batch
                w_decision_time,w_clip_decision,w_decision_up,w_out,s_out,s_decision_time, clip_level_target,E_ad,is_source = self._forward(model, batch) # 
