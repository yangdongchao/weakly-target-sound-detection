...
class Runner(object):
    ...
    def _forward(model, batch):
        ...
        decision_time, decision_up, pseudo_label, mask_loss = model(inputs, embedding, clip_level_targets)
        return decision_time, decision_up, pseudo_label, clip_level_targets, mask_loss
    ...
    def train(self, config, **kwargs):
        ...
        def _train_batch(_, batch):
            ...
            with torch.enable_grad():
                optimizer.zero_grad()
                decision_time, decision_up, pseudo_label, clip_level_target, mask_loss = self._forward(model, batch)
                ...
                # print('pseudo_label ',pseudo_label)
                # # print('clip_level_target ',clip_level_target[0])
                # assert 1==2
                # print('decision ',decision.shape)
                # print('frame_level_target ',frame_level_target.shape)
                # assert 1==2
                loss_sed = criterion(decision_time, pseudo_label)
                loss_mask = mask_loss['mask_loss']
                loss = loss_sed + 0.1*loss_mask
                loss.backward()
                # Single loss
                optimizer.step()
                return loss.item()
        ...
        def thresholded_output_transform(output):
            ...
            mat_clip = clip_level_target[:,0].unsqueeze(1).repeat(1,decision_time.shape[1])
            pseudo_label = pseudo_label*mat_clip
            y_pred  = decision_time > 0.5
            y_pred = y_pred.long()
            frame_level_target = pseudo_label > 0.5
        ...
        metrics = {
            'Loss': losses.Loss_stage3_mask(criterion),  #reimplementation of Loss, supports 3 way loss 
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                ...
                # txt_path = '/apdcephfs/share_1316500/donchaoyang/code2/data/weakly_data/soft_label/'
                # if save_soft_label:
                #     fname_target = filenames[0] +'_' + events[0] +'.txt'
                #     final_name = txt_path + fname_target
                #     np.savetxt(final_name,w_decision_time.detach().cpu().numpy()[0])
