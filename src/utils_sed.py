...
def find_contiguous_regions(activity_array): # in this part, if you cannot understand the binary operation, I think you can write a O(n) complexity method
    ...
    """
    # print('activity_array ',activity_array)
...
def _decode_with_timestamps(encoder, labels):
    result_labels = []
    #print('labels ',labels.shape)
    for i, label_column in enumerate(labels.T): # 依次遍历10类的501 frame
        #print(i,label_column)
        change_indices = find_contiguous_regions(label_column)
        # append [onset, offset] in the result list
        # print(change_indices)
        for row in change_indices:
            result_labels.append((encoder[i], row[0], row[1]))
    return result_labels

def decode_from_proposals(encoder, final_proposals):
    result_labels = []
    # print('labels ',final_proposals)
    # assert 1==2
    # print('final_proposals ',final_proposals)
    for i, label_column in enumerate(final_proposals): # 依次遍历10类的501 frame
        #print(i,label_column)
        # append [onset, offset] in the result list
        # print(change_indices)
        ...
        for row in label_column:
            k = int(row[0])
            result_labels.append((encoder.classes_[k], row[2], row[3]))
    return result_labels
...
def from_frame_to_clip(frame_target):
    clip_target = frame_target.mean(1) > 0.0
    # print(clip_target.shape)
    return clip_target