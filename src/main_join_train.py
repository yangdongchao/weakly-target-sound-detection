...
class Runner(object):
    ...
    def _forward(model, batch):
        inputs, label, embedding,embed_label, filenames,events = batch
        # print('inputs ',inputs.shape)
        # print('frame_level_targets ',frame_level_targets)
        inputs = convert_tensor(inputs, device=DEVICE, non_blocking=True)
        label = convert_tensor(label.float(), device=DEVICE, non_blocking=True)
        ...
        return decision,decision_up, label,embed_label,logit
    ...
    def train(self, config, **kwargs):
        ...
        def _train_batch(_, batch):
            ...
            with torch.enable_grad():
                optimizer.zero_grad()
                decision,decision_up, label,embed_label,logit = self._forward(model, batch)  # output is tuple (clip, frame, target)
                #decision,decision_up, label,embed_label,logit
                # print('decision ',decision.shape)
                # print('frame_level_target ',frame_level_target.shape)
                # assert 1==2
                loss_sed = criterion_sed(decision,label)
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                ...
                decision, decision_up, label, embed_label, logit = self._forward(model, batch) # 
