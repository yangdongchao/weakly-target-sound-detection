...
class Runner(object):
    ...
    def _forward(model, batch):
        ...
        decision, decision_up, mask_loss = model(inputs, embedding, clip_level_target)
        return decision, decision_up, frame_level_targets, time, mask_loss
    ...
    def train(self, config, **kwargs):
        ...
        if config_parameters['label_reverse']:
            trainloader = dataset.getdataloader_s(
        ...
        def _train_batch(_, batch):
            ...
            with torch.enable_grad():
                optimizer.zero_grad()
                decision, decision_up, frame_level_target, time, mask_loss = self._forward(model, batch)  # output is tuple (clip, frame, target)
                ...
                # print(mask_loss['mask_loss'])
                # assert 1==2
                loss = loss_sed + 0.1*mask_loss['mask_loss']
                loss.backward()
                # Single loss
                optimizer.step()
                return loss.item()
        ...
        metrics = {
            'Loss': losses.Loss_s_mask(criterion),  #reimplementation of Loss, supports 3 way loss 
    ...
    def evaluate(
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, unit='file', leave=False): # dataloard 加载了弱标签
                inputs, frame_level_targets,time, embedding, filenames,events, clip_level_target = batch
                decision,decision_up, frame_level_target, time, mask_loss = self._forward(model, batch) # 
